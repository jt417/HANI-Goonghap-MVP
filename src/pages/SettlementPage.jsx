...
export default function SettlementPage() {
  ...
  return (
    <div className="grid h-full grid-cols-[1.1fr_380px]">
      ...
      <ProposalDetailPanel title="정산 상세" item={selected} actions={<><button className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50">배분 조정</button><button className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700 hover:bg-amber-100">증빙 요청</button><button className="rounded-xl bg-slate-900 px-4 py-3 text-sm font-bold text-white hover:bg-slate-800">정산 확정</button></>} />
    </div>
  );
}
