...
export default function ProposalDetailPanel({ title, item, actions }) {
  ...
  const [localMessages, setLocalMessages] = useState(proposalMessages);
    ...
    <aside className="border-l border-slate-200 bg-white p-6 overflow-y-auto">
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="mt-4 flex gap-2">
          ...
          />
          <button onClick={handleSend} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800"><Send size={16} /></button>
        </div>
      </div>
